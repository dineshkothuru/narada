...
describe("createTables", () => {
  it("creates a batch of tables with generated unique codes, continuing numbering", async () => {
    ...
    const result = await createTables(repos, { count: 2, prefix: "Table" });
  ...
  it("creates a single labelled table", async () => {
    const { data, repos } = seed();
    const result = await createTables(repos, { label: "Patio 1" });
  ...
  it("400s without label or count", async () => {
    const { repos } = seed();
    await expect(createTables(repos, {})).rejects.toMatchObject({ statusCode: 400 });
  });
});
...
describe("patchTable", () => {
  it("updates recognised fields only", async () => {
    ...
    const result = await patchTable(repos, {
      tableId: ids.tableA,
      capacity: 6,
      ui_variant: "stories",
    });
  ...
  it("400s without a tableId", async () => {
    const { repos } = seed();
    await expect(patchTable(repos, { tableId: "" })).rejects.toMatchObject({ statusCode: 400 });
  });
});
...
describe("deleteTable", () => {
  it("blocks deleting a table with an open tab", async () => {
    ...
    await expect(deleteTable(repos, ids.tableA)).rejects.toMatchObject({ statusCode: 409 });
  });

  it("removes a table with no active session and no order history", async () => {
    const { data, repos, ids } = seed();
    const result = await deleteTable(repos, ids.tableB);
    expect(result).toEqual({ ok: true });

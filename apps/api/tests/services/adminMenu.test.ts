...
import { createMenuItem, deleteMenuItem, patchMenuItem } from "../../src/services/adminMenu.js";
import { seed } from "../helpers/fakeRepos.js";

describe("createMenuItem", () => {
  it("creates a dish under its category's outlet", async () => {
    const { data, repos, ids } = seed();
    const result = await createMenuItem(repos, {
  ...
  it("400s on a non-positive price", async () => {
    ...
    await expect(
      createMenuItem(repos, { category_id: ids.category, name: "X", price_inr: 0 }),
...
describe("deleteMenuItem", () => {
  it("removes a dish with no order history", async () => {
    const { data, repos, ids } = seed();
    const result = await deleteMenuItem(repos, ids.items[0]);
  ...
  it("400s without an itemId", async () => {
    const { repos } = seed();
    await expect(deleteMenuItem(repos, "")).rejects.toMatchObject({ statusCode: 400 });
  });
});
...
describe("patchMenuItem", () => {
  it("applies only recognised fields, filtering tags to the allow-list", async () => {
    const { data, repos, ids } = seed();
    const result = await patchMenuItem(repos, {
      itemId: ids.items[0],
      is_available: false,
  ...
  it("400s without an itemId", async () => {
    const { repos } = seed();
    await expect(patchMenuItem(repos, { itemId: "", is_available: true })).rejects.toMatchObject({
      statusCode: 400,
    });
  });
  ...
  it("400s when there is nothing valid to update", async () => {
    const { repos, ids } = seed();
    await expect(patchMenuItem(repos, { itemId: ids.items[0] })).rejects.toMatchObject({
      statusCode: 400,
    });
  });
});

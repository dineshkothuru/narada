...
// database.
export async function tipsForDay(repos: Pick<Repos, "sessions">, now: Date, outletId: string) {
  const since = istDayStart(now).toISOString();
  const sessions = await repos.sessions.listSettledSince(since, outletId);
  return {
    since,
    ...tallyTips(
      sessions.map((s) => ({

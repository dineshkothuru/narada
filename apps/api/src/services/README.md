...
- `generateBill(repos, sessionId)` takes NO tip. The counter raises a plain bill and `recordPayment` turns anything paid above it into the tip, via `splitPayment` from `@narada/shared`.

        ...
        <div className="flex max-w-5xl flex-col gap-3">
          <header className="mb-1">
            ...
            <p className="text-xs text-stone-500">
              Staff logins — each role opens only its own screens
            </p>
          </header>
            ...
            {addingStaff && (
              <form
                onSubmit={async (e) => {
                  ...
                  const res = await addStaff.mutateAsync({
                    name: String(fd.get("name")),
                    role: String(fd.get("role")),
                    pin: String(fd.get("pin")),
                  });
                  ask.toast(res.ok ? "Staff added" : "Failed");
            ...
            <div className="mt-2 divide-y divide-stone-100">
              {staff.length === 0 && !addingStaff && (
                <p className="py-3 text-xs text-stone-400">
                  No staff yet — everyone is using the owner PIN. Add waiters and cooks so each has
                  their own PIN, and their PIN only opens their screen.
                </p>
              )}
              {staff.map((s) => (

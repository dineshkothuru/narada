...
export default function TipsBoard() {
  ...
  if (!tips) return <p className="text-xs text-slate-400">Loading…</p>;

  if (tips.rows.length === 0 && tips.unassigned === 0) {
    return <p className="text-xs text-slate-400">No tips collected yet today.</p>;
  }
    ...
    <div className="flex flex-col gap-1.5">
      {tips.rows.map((r) => (
        ...
        >
          <span className="min-w-0">
            <span className="text-sm font-semibold text-slate-800">👤 {r.attendant}</span>
      ...
      {tips.unassigned > 0 && (
        <div className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2.5">
          ...
          <span className="text-sm font-bold text-slate-500">{inr(tips.unassigned)}</span>
        </div>
      )}

...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (body.action === "mark_paid" && body.sessionId) {
      const sessions = await sbFetch<{ id: string; restaurant_id: string; table_id: string }[]>(
        `sessions?select=id,restaurant_id,table_id&id=eq.${encodeURIComponent(body.sessionId)}&limit=1`,
      );
